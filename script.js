...
const Auth = {
  ...
  init() {
    if (!window.firebase || !window.MITTSU_FIREBASE_CONFIG || window.MITTSU_FIREBASE_CONFIG.apiKey === 'PASTE_YOUR_FIREBASE_API_KEY') {
      console.warn('Firebase is not configured yet. Add your values to firebase-config.js.');
      this.updateNavUI();
      return;
    }
    try {
      if (!firebase.apps.length) firebase.initializeApp(window.MITTSU_FIREBASE_CONFIG);
      ...
      firebase.auth().onAuthStateChanged(user => {
        ...
        this.session = {
          uid: user.uid,
          email,
          role: email === AUTHOR_EMAIL ? 'author' : 'reader',
      ...
      console.error(e);
      toast('Firebase could not be initialized. Check firebase-config.js.');
    }
  },

  isAuthor() { return !!(this.session && this.session.role === 'author' && this.session.email === AUTHOR_EMAIL); },

  requireAuthor() {
    if (this.isAuthor()) return true;
    toast('Only the Mittsu Writers admin account can edit stories and chapters.');
    if (!this.session) this.openAuth('login');
    return false;
  },
  ...
  async logout() {
    try {
      if (this.firebaseReady) await firebase.auth().signOut();
  ...
  updateNavUI() {
    ...
    const isAuthorNow = this.isAuthor();
    ...
    if (loggedIn) {
      const label = isAuthorNow ? '✎ Mittsu Writers' : (this.session.displayName || this.session.email);
  ...
  openAuth(mode) {
    if (this.session) { toast(`You're already logged in as ${this.session.email}`); return; }
    $('#authModal').classList.remove('hidden');
    document.body.classList.add('locked');
    this.renderForm(mode || 'login');
  },
  ...
  firebaseError(err) {
    ...
    toast(messages[code] || (err && err.message ? err.message : 'Authentication failed.'));
  },
  ...
  renderForm(mode) {
    ...
    if (isForgot) {
      $('#authModalContent').innerHTML = `
        ...
        <p class="modal-sub">Enter your email and Firebase will send you a secure password-reset link.</p>
        <div class="form-field"><label>Email</label><input type="email" id="authEmail" placeholder="you@example.com" autocomplete="email"></div>
        <button class="btn btn-solid btn-block" id="authResetBtn">Send reset email</button>
        <p class="modal-switch"><button id="authBackBtn">← Back to login</button></p>
        <p class="modal-note">Password reset is available for both readers and the Mittsu Writers admin account.</p>`;
      $('#authResetBtn').addEventListener('click', () => this.sendPasswordReset());
      $('#authBackBtn').addEventListener('click', () => this.renderForm('login'));
      $('#authEmail').addEventListener('keydown', e => { if (e.key === 'Enter') this.sendPasswordReset(); });
      return;
    }
    $('#authModalContent').innerHTML = `
      <h3>${isLogin ? 'Log in' : 'Create account'}</h3>
      <p class="modal-sub">${isLogin ? 'Welcome back to Mittsu Writers.' : 'Create an account to like, vote and comment on stories.'}</p>
      <div class="form-field"><label>Email</label><input type="email" id="authEmail" placeholder="you@example.com" autocomplete="email"></div>
      ...
      ${isLogin ? '<button class="btn btn-ghost btn-block google-auth-btn" id="googleAuthBtn">Continue with Google</button>' : ''}
      ${isLogin ? '<p style="text-align:center;margin:12px 0 0"><button class="text-link" id="forgotPasswordBtn">Forgot password?</button></p>' : ''}
      <p class="modal-switch">${isLogin ? 'New here?' : 'Already have an account?'} <button id="authSwitchBtn">${isLogin ? 'Sign up' : 'Log in'}</button></p>
      <p class="modal-note">Only the Mittsu Writers admin account can add, edit, or delete stories and chapters. Everyone else can read, like, vote, and comment.</p>`;
    $('#authSubmitBtn').addEventListener('click', () => isLogin ? this.submitLogin() : this.submitSignup());
    $('#authSwitchBtn').addEventListener('click', () => this.renderForm(isLogin ? 'signup' : 'login'));
    if (isLogin) {
      $('#googleAuthBtn').addEventListener('click', () => this.signInWithGoogle());
      $('#forgotPasswordBtn').addEventListener('click', () => this.renderForm('forgot'));
    }
    $all('#authModalContent input').forEach(inp => inp.addEventListener('keydown', e => {
      if (e.key === 'Enter') (isLogin ? this.submitLogin() : this.submitSignup());
    }));
  },

  async submitSignup() {
    if (!this.firebaseReady) { toast('Connect Firebase first using firebase-config.js.'); return; }
    const email = $('#authEmail').value.trim().toLowerCase();
    const password = $('#authPassword').value;
    if (!email || !password) { toast('Enter an email and password'); return; }
    if (email === AUTHOR_EMAIL) {
      toast('The admin account is created in Firebase by the site owner. Use Log in instead.');
  ...
  async signInWithGoogle() {
    ...
    try {
      ...
      if (email === AUTHOR_EMAIL) {
        ...
        toast('Signed in with Google. Welcome to Mittsu Writers.');
      }
    } catch (e) { this.firebaseError(e); }
  },
